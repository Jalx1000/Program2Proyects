...
void TForm1::Cargar(String nom)
{
    ...
    if (!pf.fail()) { //si logró abrir el archivo
        ...
        while (!pf.eof()) {
            ...
            switch (img) {
                case '#':
					ImageList1->Draw(Canvas, j * widht, i * height, 0);
					break;
                case 'h':
					ImageList1->Draw(Canvas, j * widht, i * height, 1);
                    x = j;
                    y = i;
                    break;
				case 'p':
					ImageList1->Draw(Canvas, j * widht, i * height, 2);
                    break;
				case ' ':
					ImageList1->Draw(Canvas, j * widht, i * height, 3);
					break;
                case 10:
                    n = j;
                    i++;
...
bool TForm1::BackTrack(char Lab[][100], int x, int y)
{
    ...
    nc = 0;
    if ((Lab[x + 1][y] == ' ') ||
        (Lab[x + 1][y] == 'p')) //si hay camino a la derecha
        caminos[nc++] = '6';
    if ((Lab[x - 1][y] == ' ') ||
        (Lab[x - 1][y] == 'p')) //si hay camino a la izquierda
        caminos[nc++] = '4';
    ...
    while ((nc > 0) && (!haySalida)) {
		...
		ImageList1->Draw(Canvas, x * widht, y * height, 3); //borra al hamster
        switch (d) {
            case '4':
                x--;
                break;
		...
		ImageList1->Draw(Canvas, x * widht, y * height, 1); //dibuja al hamster
        Sleep(100);
        if (Lab[x][y] == 'p')
            haySalida = TRUE;
        else {
